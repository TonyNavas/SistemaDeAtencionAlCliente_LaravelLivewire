<div style="max-width: 95%; padding-bottom: 7rem;" class="container">
    <div class="text-center mb-5">
        <h3 class="fw-semibold">CONTENIDO</h3>
    </div>
    <div class="row">
        <div class="col-lg-3">
            <div style="border: 0;" style="width: 18rem;">
                <img style="max-height: 200px;" src="{{ asset('img/menu.jpg') }}" class="card-img-top rounded"
                    alt="...">
                <div class="card-body mt-3 text-center">
                    <h5 class="fw-semibold">Menú en línea</h5>
                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit.<a class="btn text-primary fw-semibold"
                            style="font-style: italic;" href="{{ route('menu') }}">Ver menu</a></p>
                </div>
            </div>
        </div>
        <div class="col-lg-3">
            <div style="border: 0;" style="width: 18rem;">
                <img style="max-height: 200px;" src="{{ asset('img/notification.jpg') }}" class="card-img-top rounded"
                    alt="...">
                <div class="card-body mt-3 text-center">
                    <h5 class="fw-semibold">Timbre en línea</h5>
                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit.<a class="btn text-primary fw-semibold"
                            style="font-style: italic;" href="{{ route('home') }}">Solicitar atención</a>
                    </p>
                </div>
            </div>
        </div>
        <div class="col-lg-3">
            <div style="border: 0;" style="width: 18rem;">
                <img style="max-height: 200px;" src="{{ asset('img/services.jpg') }}" class="card-img-top rounded"
                    alt="...">
                <div class="card-body mt-3 text-center">
                    <h5 class="fw-semibold">Servicios</h5>
                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit.<a class="btn text-primary fw-semibold"
                            style="font-style:italic;" href="{{ route('menu') }}">Solicita nuestros servicios</a>
                    </p>

                </div>
            </div>
        </div>
        <div class="col-lg-3">
            <div style="border: 0;" style="width: 18rem;">
                <img style="max-height: 200px;" src="{{ asset('img/galery.jpg') }}" class="card-img-top rounded"
                    alt="...">
                <div class="card-body mt-3 text-center">
                    <h5 class="fw-semibold">Galeria</h5>
                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit.<a class="btn text-primary fw-semibold"
                        style="font-style: italic;" href="{{ route('menu') }}">Echa un vistazo</a>
                    </p>
                </div>
            </div>
        </div>
    </div>
</div>
<figure style="margin-bottom: 7rem;">
    <img style="max-height: 300px; width: 100%;" src="{{ asset('img/4.jpeg') }}" alt="">
</figure>
